...
such process. Now, let $\mathbf{Y}$ to be a random variable observed at a
...
\perp U$.  The expected value for $Y( A_i )$ is
\[
E[ Y ( A_i  )  ] = \alpha,
\]
while its variance/covarianca matrix is given as follows
\begin{align*}
...
valid as long as the integrals are well defined (not equal to
infinity). Additionally, when $i = j$, the variance at the region $A_i$ is
\[
...
## $p$-dimensional $Y$ and univariate $X$
...
\end{array}
\]
where $\Omega$ is a $p \times p$ matrix with entries $\omega_{ij}$.
...
\]
Therefore, the covariance matrix associated with the random variable
$\mathbf{Y}$ is
\[
...
Moreover, the following "cross" covariance relationship between $Y_{j}$ and $X$
has entries
\[
(D_j)_{ik} = Cov(Y_{ij}, X_{k}) = \beta_j f(A_i, B_k ; \theta),
\]
where $D_j$ is $n \times m$. Thus, we define the "cross"-covariance as
\[
D = \begin{pmatrix}
D_1 & \cdots & D_p
...
We can write the joint log-likelihood as
\[
