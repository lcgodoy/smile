...
region $\mathrm{D} \in {\cal R}^2$ (e.g., a city). This partition is denoted by
...
Consequently,
\begin{align*}
\mathrm{E}[Y(A_i)] & = \frac{1}{\lvert A_i \rvert} \int_{A_i} {\rm
E}[Y(\mathbf{s})] \, \mathrm{d} \mathbf{s} \\
& = \frac{1}{\lvert A_i \rvert} \int_{A_i} \mu \, \mathrm{d} \mathbf{s} \\
& = \mu,
\end{align*}
and
\begin{align*}
...
thus,
$$
Y(A_1, \cdots, A_n) \sim \mathrm{N}( \mu \mathbf{1}_n, \sigma^2 {\rm
R}_{\kappa}(\phi) + \tau \mathrm{diag}(\lvert A_1 \rvert^{-1}, \ldots, \lvert A_1
\rvert^{-1})).
$$
