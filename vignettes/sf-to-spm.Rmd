...
This vignette is useful when the used wants to pursue the model-based
approach. This methodology is based on the assumption that areal data (e.g.,
variables measured at census tracts) is composed by averages of a continuous
underlying process [gelfand2001change, @moraga2017geostatistica,
@johnson2020dealing, @wilson2020pointless]. In practice, there exists an
identifiability when estimating some variance parameters, which can be seen
either as small scale variation (nugget effect) or measurement errors. When
...
Liverpool. These variables are Middle Super Output Areas (MSOA) and in the Lower
Super Output Areas (LSOA), respectively. After loading our package, these
datasets can be loaded by simply running the chunk of code below. Figure
...
```{R read-data, fig.caption="LEB in Liverpool at the MSOA."}
...
\lVert \mathbf{s} - \mathbf{s}' \rVert ; \bolsymbol{\theta}) \, {\rm d}
\mathbf{s} \, {\rm d} \mathbf{s'},
\end{align*}
...
\mathbf{s} - \mathbf{s}' \rVert ; \bolsymbol{\theta})$ is a isotropic covariance
function depending on the parameter $\boldsymbol{\theta}$.
...
numerically or by Monte Carlo integration [@gelfand2001chnge]. When using the
...
that for which we will be interested in fitting a model to. Finally, Figure
\ref{fig:sf-to-spm2} displays the grids associated with each of the possible
combinations of the parameters `type` and `by_polygon` when calling the
`sf_to_spm` function.
```{R sf-to-spm1}
msoa_spm <-
...
```{R sf-to-spm2, echo = FALSE, warning = FALSE, message = FALSE, fig.caption = "All the possible combinations for the arguments `type` and `by_polygon`,"}
set.seed(123)
